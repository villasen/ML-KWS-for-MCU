...
void RespondToCommand(tflite::ErrorReporter *error_reporter,
  ...
  if (is_new_command) {
    ...
    if (*found_command == 'y') {
      ...
      lcd.DisplayStringAt(0, LINE(5), (uint8_t *)"Houston Heard yes!", CENTER_MODE);
    } else if (*found_command == 'n') {
      lcd.Clear(0xFFDB4437);
      lcd.DisplayStringAt(0, LINE(5), (uint8_t *)"Houston Heard no :(", CENTER_MODE);
    } else if (*found_command == 'u') {
      lcd.Clear(0xFFF4B400);
      lcd.DisplayStringAt(0, LINE(5), (uint8_t *)"Houston Heard unknown", CENTER_MODE);
    } else {
      lcd.Clear(0xFF4285F4);
      lcd.DisplayStringAt(0, LINE(5), (uint8_t *)"Houston Heard silence", CENTER_MODE);
    }
